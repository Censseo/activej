...
/**
 * An {@link IUdpSocket} over {@link StubDatagramNetwork}.
 * <p>
 * This is the whole reason the HTTP/3 wire assertions are trustworthy: {@code IUdpSocket} is already a
 * three-method interface, so the harness plugs in at an existing seam and <b>no production code
 * changes and no test-only hook in the transport are needed</b>. The {@code QuicEndpoint},
 * {@code QuicConnection} and {@code QuicStreamManager} under an {@link Http3WirePair} are
 * byte-identical to the ones that run over a real socket — the same trick {@code StubHttpClient} plays
 * for HTTP.
 * <p>
...
public final class StubUdpSocket implements IUdpSocket {
	...
	public StubUdpSocket(StubDatagramNetwork network, InetSocketAddress localAddress) {
	...
	public String toString() {
		return "StubUdpSocket{" + localAddress + ", inbox=" + inbox.size() + (closed ? ", closed" : "") + '}';
	}
}
