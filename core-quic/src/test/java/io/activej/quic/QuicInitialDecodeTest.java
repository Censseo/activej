/*
 * Copyright (C) 2020 ActiveJ LLC.
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 * http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

package io.activej.quic;

import io.activej.bytebuf.ByteBuf;
import io.activej.bytebuf.ByteBufPool;
import io.activej.quic.codec.CryptoFrame;
import io.activej.quic.codec.QuicFrame;
import io.activej.quic.codec.QuicFrames;
import io.activej.quic.crypto.InitialKeys;
import io.activej.quic.crypto.QuicKeys;
import io.activej.quic.crypto.QuicPacketProtection;
import io.activej.test.rules.ByteBufRule;
import org.junit.ClassRule;
import org.junit.Test;

import java.util.ArrayList;
import java.util.HexFormat;
import java.util.List;

import static org.junit.Assert.*;

/**
 * Capstone end-to-end test: the full receive pipeline on RFC 9001 Appendix A.2's protected
 * client Initial packet — peek the DCID from the raw datagram, derive Initial keys (T022),
 * remove packet protection (T024), and decode the resulting plaintext into frames (T013). This
 * is the composition point for every ownership-transfer contract established across Phases 2-5.
 */
public class QuicInitialDecodeTest {

	@ClassRule
	public static final ByteBufRule byteBufRule = new ByteBufRule();

	private static final String PKT_A2_HEX = "";

	@Test
	public void decodesRfc9001AppendixA2ClientInitialEndToEnd() throws Exception {
		byte[] rawDatagram = HexFormat.of().parseHex(PKT_A2_HEX);
		ByteBuf datagram = ByteBufPool.allocate(rawDatagram.length);
		datagram.put(rawDatagram);

		// 1. Peek the DCID directly from the raw (still-protected) datagram: the long-header
		// envelope's form/version/connection-ID layout is never protected (RFC 8999/9001 §5.4.1).
		QuicConnectionId dcid = peekDestinationConnectionId(datagram);

		// 2. Derive Initial keys from that DCID (T022).
		InitialKeys initialKeys = QuicKeys.initial(dcid);
		QuicKeys clientKeys = initialKeys.client();

		// 3. Remove packet protection (T024): AEAD-open the client Initial packet.
		QuicPacketProtection.OpenResult opened = QuicPacketProtection.open(clientKeys, -1, dcid.length(), datagram);
		assertEquals(2L, opened.packetNumber);

		// 4. Decode the plaintext into frames (T013) until the buffer is exhausted.
		List<QuicFrame> frames = new ArrayList<>();
		while (opened.payload.canRead()) {
			frames.add(QuicFrames.read(opened.payload));
		}

		assertFalse(frames.isEmpty());
		assertTrue("first frame must be the ClientHello's CRYPTO frame", frames.get(0) instanceof CryptoFrame);
		CryptoFrame crypto = (CryptoFrame) frames.get(0);
		assertEquals(0, crypto.offset);
		assertTrue("CRYPTO frame payload must start with a TLS ClientHello (handshake type 1)", crypto.payload.canRead());
		assertEquals(0x01, crypto.payload.peek());

		for (QuicFrame frame : frames) {
			if (frame instanceof io.activej.common.recycle.Recyclable recyclable) {
				recyclable.recycle();
			}
		}
		opened.payload.recycle();
	}

	/**
	 * Reads just enough of a long-header packet's invariant envelope (RFC 8999) to find the
	 * Destination Connection ID, without disturbing {@code datagram}'s read position — the
	 * real {@link QuicPacketProtection#open} call afterward needs the datagram untouched.
	 */
	private static QuicConnectionId peekDestinationConnectionId(ByteBuf datagram) {
		byte[] array = datagram.array();
		int base = datagram.head();
		int dcidLength = array[base + 5] & 0xFF;
		byte[] dcidBytes = java.util.Arrays.copyOfRange(array, base + 6, base + 6 + dcidLength);
		return QuicConnectionId.of(dcidBytes);
	}
}
