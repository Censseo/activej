 ...
 * <p>
 * <b>Why this exists.</b> {@code EventloopRule} builds its eventloop on the system clock, so a
 * connection's {@code reactor.delay(...)} deadlines live on real time while {@link DatagramNetwork}'s
 * delivery lives on the fixture clock. The two cannot be stepped together, which makes every
 * timer-driven behaviour — loss detection, probe timeouts, the closing period, the idle timeout —
 * untestable without sleeping. Building the eventloop on the <i>same</i> clock as the wire is what
 * makes those deterministic (FR-039, SC-011).
 * <p>
 ...
 * <p>
 * <b>Always close it</b> (try-with-resources): {@code close()} restores the previous current reactor,
...
public final class ManualEventloop implements AutoCloseable {
	...
	public ManualEventloop(long startMillis) {
		...
		this.clock = QuicTestPeers.settableClock(startMillis);
