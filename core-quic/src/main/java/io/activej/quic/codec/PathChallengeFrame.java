...
/**
 * PATH_CHALLENGE frame (RFC 9000 §19.17). {@link #data} is always exactly 8 bytes.
 */
public final class PathChallengeFrame extends QuicFrame {
	...
	public PathChallengeFrame(byte[] data) {
		if (data.length != DATA_LENGTH) {
			throw new IllegalArgumentException("PATH_CHALLENGE data must be " + DATA_LENGTH + " bytes: " + data.length);
		}
		this.data = data.clone();
	}

	/** Defensive copy of the 8-byte challenge payload. */
	public byte[] data() {
		return data.clone();
	}
	...
	public boolean equals(Object o) {
		...
		if (!(o instanceof PathChallengeFrame other)) return false;
